 ...
 *      - {String} network (required) the network id. Default to false.
 *      - {Boolean} primary (optional) default to false.
 * @param {Function} callback of the form f(err, nic, res).
 *
 * The primary flag depends on the cloudapi endpoint being new enough to support
 * PUBAPI-1216.
 */
...
function addNic(opts, cb) {
    assert.object(opts, 'opts');
    assert.uuid(opts.id, 'opts.id');
    assert.uuid(opts.network, 'opts.network');
