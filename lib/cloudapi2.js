 ...
 * @param {Function} cb `function (err, accesskeys, res)`
 */
CloudApi.prototype.listAccessKeys = function listAccessKeys(opts, cb) {
    if (cb === undefined) {
        cb = opts;
        opts = {};
    }
    ...
    var endpoint = format('/%s/accesskeys', this.account);
    this._passThrough(endpoint, opts, cb);
};

/**
 * Create a new access key (with secret) for the account.
 *
...
CloudApi.prototype.createAccessKey = function createAccessKey(opts, cb) {
    ...
    this._request({
        method: 'POST',
        path: format('/%s/accesskeys', this.account),
        data: opts
    }, function (err, req, res, body) {
        cb(err, body, res);
    });
 ...
 *      - {String} accessKeyId Required access key identifier.
 * @param {Function} cb `function (err, res)`
 */
CloudApi.prototype.deleteAccessKey = function deleteAccessKey(opts, cb) {
    assert.object(opts, 'opts');
    ...
    this._request({
        method: 'DELETE',
        path: format('/%s/accesskeys/%s',
            this.account, opts.accessKeyId)
    }, function (err, req, res) {
        cb(err, res);
    });
};
...
/**
 * Update an access key for the account.
 *
 * @param {Object} opts object containing:
 ...
 * @param {Function} cb `function (err, accesskey, res)`
 */
CloudApi.prototype.updateAccessKey = function updateAccessKey(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.accessKeyId, 'opts.accessKeyId');
    ...
    var data = {};
    Object.keys(this.UPDATE_ACCESSKEY_FIELDS).forEach(function (attr) {
        if (opts[attr] !== undefined) {
            data[attr] = opts[attr];
        }
    });
    ...
    this._request({
        method: 'POST',
        path: format('/%s/accesskeys/%s', this.account, opts.accessKeyId),
        data: data
    }, function (err, req, res, body) {
        cb(err, body, res);
    });
...
/**
 * Get an access key from the account.
 *
 * @param {Object} opts Parameters.
 *      - {String} accessKeyId Required access key identifier.
 * @param {Function} cb `function (err, accesskey, res)`
 */
CloudApi.prototype.getAccessKey = function getAccessKey(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.accessKeyId, 'opts.accessKeyId');
    assert.func(cb, 'cb');

    var endpoint = format('/%s/accesskeys/%s',
        this.account, opts.accessKeyId);
    this._passThrough(endpoint, {}, cb);
};

/**
 * Create a new access key (with secret) for an RBAC user.
 *
 * @param {Object} opts
...
CloudApi.prototype.createUserAccessKey =
    function createUserAccessKey(opts, cb) {
    assert.object(opts, 'opts');
    ...
    this._request({
        method: 'POST',
        path: format('/%s/users/%s/accesskeys', this.account, opts.userId),
        data: opts
    }, function (err, req, res, body) {
        cb(err, body, res);
    });
...
/**
 * Get an access key for an RBAC user.
 *
 * @param {Object} opts Parameters.
 *      - {String} userId: Required.
 *      - {String} accessKeyId Required access key identifier.
 * @param {Function} cb `function (err, accesskey, res)`
 */
CloudApi.prototype.getUserAccessKey = function getUserAccessKey(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.accessKeyId, 'opts.accessKeyId');
    assert.string(opts.userId, 'opts.userId');
    assert.func(cb, 'cb');

    var endpoint = format('/%s/users/%s/accesskeys/%s',
        this.account, opts.userId, opts.accessKeyId);
    this._passThrough(endpoint, {}, cb);
};

/**
 * List account access keys for an RBAC user.
 *
 * @param {Object} opts Parameters.
 *      - {String} userId: Required.
 * @param {Function} cb `function (err, accesskey, res)`
 */
CloudApi.prototype.listUserAccessKeys = function listUserAccessKeys(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.userId, 'opts.userId');
    assert.func(cb, 'cb');

    var endpoint = format('/%s/users/%s/accesskeys',
        this.account, opts.userId);
    this._passThrough(endpoint, {}, cb);
};

/**
 * Delete an access key for an RBAC user.
 *
 * @param {Object} opts Parameters.
 *      - {String} userId: Required.
 *      - {String} accessKeyId Required access key identifier.
 * @param {Function} cb `function (err, res)`
 */
CloudApi.prototype.deleteUserAccessKey =
    function deleteUserAccessKey(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.userId, 'opts.userId');
    assert.string(opts.accessKeyId, 'opts.accessKeyId');
    assert.func(cb, 'cb');
    ...
    this._request({
        method: 'DELETE',
        path: format('/%s/users/%s/accesskeys/%s',
            this.account, opts.userId, opts.accessKeyId)
    }, function (err, req, res) {
        cb(err, res);
    });
...
/**
 * Update access key for an RBAC user.
 *
 * @param {Object} opts object containing:
 *      - {String} userId: Required.
 *      - {String} accessKeyId: Required.
 ...
 * @param {Function} cb `function (err, accesskey, res)`
 */
CloudApi.prototype.updateUserAccessKey =
    function updateUserAccessKey(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.accessKeyId, 'opts.accessKeyId');
    assert.string(opts.userId, 'opts.userId');
    assert.optionalString(opts.status, 'opts.status');
 ...
 *      - {String} mechanism (optional) 'cold' (default) or 'live'.  Only
 *        meaningful on 'begin' / 'automatic'; later phases inherit the
...
function machineMigration(opts, cb) {
    ...
    assert.optionalString(opts.mechanism, 'opts.mechanism');
    assert.func(cb, 'cb');
    ...
    if (opts.mechanism !== undefined) {
        // Validate the value regardless of action so typos surface
        // before we hit the network — even a typo on a sync/switch
        // call deserves a clear local error, not a silent accept
        // followed by a server-side PreconditionFailed later.
        if (opts.mechanism !== 'cold' && opts.mechanism !== 'live') {
            cb(new errors.TritonError('Invalid mechanism "' +
                        opts.mechanism + '" (expected cold or live)'));
            return;
        }
        // But only FORWARD it on the begin-class actions: VMAPI freezes
        // mechanism on the migration record at begin time; later phases
        // inherit it and rejecting a redundant value upstream would be
        // surprising to a caller who passed --mechanism on every
        // invocation out of habit.
        if (opts.action === 'begin' || opts.action === 'automatic') {
            data.mechanism = opts.mechanism;
        }
    }
