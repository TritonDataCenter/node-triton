 ...
 *
 * @param {Object} opts Optional payload (currently unused).
 ...
 *      - accessKeyId {String} Required access key identifier.
...
CloudApi.prototype.deleteAccessKey = function deleteAccessKey(opts, cb) {
    ...
    this._request({
        method: 'DELETE',
        path: format('/%s/accesskeys/%s', this.account,
            encodeURIComponent(opts.accessKeyId))
    }, function (err, req, res) {
        cb(err, res);
    });
};
...
CloudApi.prototype.getAccessKey = function getAccessKey(opts, cb) {
    ...
    var endpoint = format('/%s/accesskeys/%s', this.account,
        encodeURIComponent(opts.accessKeyId));
    this._passThrough(endpoint, {}, cb);
};
