...
function do_update(subcmd, opts, args, cb) {
    ...
    vasync.pipeline({arg: {cli: this.top}, funcs: [
        ...
        function gatherDataArgs(ctx, next) {
            try {
                ctx.data = common.objFromKeyValueArgs(args, {
                    disableDotted: true,
                    typeHintFromKey: UPDATE_IMAGE_FIELDS
                });
            } catch (err) {
                next(err);
                return;
        ...
        function validateIt(ctx, next) {
            try {
                common.validateObject(ctx.data, UPDATE_IMAGE_FIELDS);
            } catch (e) {
                next(e);
                return;
            }
...
do_update.help = [
    ...
    'Updateable fields:',
    '    ' + Object.keys(UPDATE_IMAGE_FIELDS).sort().map(function (f) {
        return f + ' (' + UPDATE_IMAGE_FIELDS[f] + ')';
    }).join('\n    '),
    '',
