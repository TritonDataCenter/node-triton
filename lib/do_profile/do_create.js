    ...
    vasync.pipeline({arg: {}, funcs: [
        ...
        function gatherDataInteractive(ctx, next) {
            ...
            if (ctx.copy) {
                ...
                defaults.url = 'https://us-central-1.api.mnx.io';
            }
        ...
        function dockerSetup(ctx, next) {
            ...
            var chosenKey = ctx.keyChoices.find(function (keyChoice) {
                return (data.keyId === keyChoice.keyId);
            });

            if (chosenKey.pubKey.type === 'ed25519') {
                console.log(common.ansiStylizeTty('\n\n# Skipping Docker ' +
                    'Setup\n', 'bold'));
                console.log(wrap80('You have selected an ED25519 key. ' +
                    'ED25519 keys can not be used for generating Docker ' +
                    'client certificates until EDDSA is adopted by the ' +
                    'CA/Browser Forum. Use an RSA or ECDSA key to generate ' +
                    'a Docker client certificate.\n'));
                next();
                return;
            }
