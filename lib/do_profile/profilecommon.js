...
function portalUrlFromCloudapiUrl(url) {
    ...
    var MNX_RE = /^https:\/\/([a-z0-9-]+)\.api\.mnx\.io\/?$/;
    if (JPC_RE.test(url)) {
        return 'https://my.joyent.com';
    } else if (MNX_RE.test(url)) {
        return 'https://my.mnx.io';
    }
    return portalUrl;
}
    ...
    vasync.pipeline({arg: {tritonapi: tritonapi}, funcs: [
        ...
        function getSigningKey(arg, next) {
            ...
            agent.listKeys(function (err, keys) {
                ...
                if (!arg.certSignKey) {
                    next(new Error('No key found in the SSH auth agent ' +
                        'matching the key fingerprint in the profile.'));
                    return;
                }
                next();
            });
        ...
        function generateAndSignCert(arg, next) {
            ...
            if (certSignKey.type === 'ed25519') {
                console.log(wordwrap('\nThe provided Triton profile: "' +
                    opts.name + '" is using an ED25519 key which can not yet ' +
                    'be used for generating CMON certificates.\n\n' +
                    'Use --keyId to select an ECDSA key for signing your ' +
                    'CMON certificate.'));
                next(new Error('ED25519 not supported.'));
                return;
            }
    ...
    vasync.pipeline({arg: {tritonapi: tritonapi}, funcs: [
        ...
        function getSigningKey(arg, next) {
            ...
            kr.findSigningKeyPair(profileFp,
                ...
                if (keyPair.getPublicKey().type === 'ed25519') {
                    console.log(wordwrap('\nThe provided Triton profile: "' +
                        opts.name + '" is using an ED25519 key which can not ' +
                        'yet be used for generating Docker client ' +
                        'certificates.\n\nUse --keyId to select an ECDSA or ' +
                        'RSA key for signing your Docker certificate.'));
                    next(new Error('ED25519 not supported.'));
                    return;
                }
