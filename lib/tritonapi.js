...
function listMigrations(opts, cb) {
    assert.func(cb, 'cb');
    var self = this;
    self.cloudapi.listMigrations(cb);
};

/**
 * Get instance migration estimate
 *
 * @param {Object} opts
 *      - {String} id: The instance ID, name, or short ID. Required.
 * @param {Function} callback `function (err, migration, res)`
*/
TritonApi.prototype.getMigrationEstimate =
function getMigrationEstimate(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.id, 'opts.id');
    assert.func(cb, 'cb');
    var self = this;
    ...
    vasync.pipeline({arg: {client: self, id: opts.id}, funcs: [
        _stepInstId,
        function _getEstimate(arg, next) {
            self.cloudapi.machineMigationEstimate({
                id: arg.instId
            }, function (err, est, _res) {
                    res = _res;
                    res.instId = arg.instId; // same hack
                    estimate = est;
                    next(err);
                });
        }
    ]}, function (pipeErr) {
        cb(pipeErr, estimate, res);
    });
