...
function _stepFabricNetId(arg, next) {
    ...
    arg.client.getNetwork(id, function (err, net) {
        ...
        arg.client.getFabricVlan(vlanId, function (err2, vlan) {
            if (err2) {
                return next(err2);
            }
...
function _stepFabricVlanId(arg, next) {
    ...
    arg.client.getFabricVlan(vlanId, function (err, vlan) {
        if (err) {
            return next(err);
        }
 ...
 * @param {Object} opts
 *      - {String} id: The instance ID, name, or short ID. Required.
 *      - {String} network: The network ID, name, or short ID. Required.
 *      - {Boolean} primary: Whether this NIC should be the instance's primary.
 *          Optional.
...
function addNic(opts, cb) {
    assert.string(opts.id, 'opts.id');
    assert.string(opts.network, 'opts.network');
...
TritonApi.prototype.getFabricVlan = function getFabricVlan(name, cb) {
    ...
    if (+name >= 0 && +name < 4096) {
        this.cloudapi.getFabricVlan({vlan_id: +name}, function (err, vlan) {
            if (err) {
                if (err.restCode === 'ResourceNotFound') {
        ...
        this.cloudapi.listFabricVlans({}, function (err, vlans) {
            if (err) {
                return cb(err);
            }
...
TritonApi.prototype.deleteFabricVlan = function deleteFabricVlan(opts, cb) {
    ...
    if (+vlanId >= 0 && +vlanId < 4096) {
        ...
        self.getFabricVlan(vlanId, function (err, vlan) {
            if (err) {
                cb(err);
                return;
            }
