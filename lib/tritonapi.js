...
/**
 * A function appropriate for `vasync.pipeline` funcs that takes a `arg.vpc_id`,
 * where `arg.vpc_id` is a VPC name, shortid, uuid. Sets the VPC UUID as
 * `arg.vpcId`.
 */
function _stepVPCId(arg, next) {
    assert.object(arg, 'arg');
    assert.object(arg.client, 'arg.client');
    assert.string(arg.vpc_id, 'arg.vpc_id');

    var vpcId = arg.vpc_id;
    ...
    arg.client.getVPC(vpcId, function onGet(err, vpc) {
        if (err) {
            next(err);
            return;
        }

        arg.vpcId = vpc.vpc_id;
        next();
    });
}
...
/**
 * List all of the networks on a VPC. Takes a VPC's UUID, short name, or name
 * as an argument.
 */
TritonApi.prototype.listVPCNetworks =
function listVPCNetworks(opts, cb) {
    ...
    vasync.pipeline({
        arg: {client: self, vpc_id: opts.vpc_id}, funcs: [
        _stepVPCId,

        function listNetworks(arg, next) {
            self.cloudapi.listVPCNetworks({
                vpc_id: arg.vpcId
            }, function listCb(err, nets) {
                if (err) {
                    next(err);
...
TritonApi.prototype.deleteVPCNetwork =
function deleteVPCNetwork(opts, cb) {
    ...
    funcs: [
        ...
        function deleteNetwork(arg, next) {
            self.cloudapi.deleteVPCFabricNetwork({
                id: arg.netId, vlan_id: arg.vlanId
            }, next);
        }
    ]}, cb);
...
TritonApi.prototype.getVPC = function getVPC(id, cb) {
    assert.string(id, 'id');
    assert.func(cb, 'cb');

    if (common.isUUID(id)) {
        this.cloudapi.getVPC(id, function getCb(err, vpc) {
            if (err) {
                cb(err);
                return;
            }
    ...
    this.cloudapi.listVPCs({}, function listCb(err, vpcs) {
        if (err) {
            cb(err);
            return;
        }

        var shortIdMatches = vpcs.filter(function shortIdCb(vpc) {
            return vpc.id.slice(0, 8) === id;
        });

        var nameMatches = vpcs.filter(function nameCb(vpc) {
            return vpc.hasOwnProperty('name') && vpc.name === id;
        });
        ...
        if (shortIdMatches.length === 0 && nameMatches.length === 0) {
            cb(new errors.ResourceNotFoundError(
                format('no VPC with name or short id "%s" was found', id)));
            return;
        }

        cb(new errors.ResourceNotFoundError(
            format('"%s" is an ambiguous VPC identifier with multiple ' +
                'matching names', id)));
    });
};

TritonApi.prototype.deleteVpc = function deleteVPC(opts, cb) {
    assert.string(opts.id, 'opts.id');
    assert.func(cb, 'cb');
    ...
    if (common.isUUID(vpcId)) {
        self.cloudapi.deleteVPC({vpc_id: vpcId}, cb);
        return;
    }

    self.getVPC(vpcId, function onGet(err, vpc) {
        if (err) {
            cb(err);
            return;
        }
