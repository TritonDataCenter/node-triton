...
function do_delete(subcmd, opts, args, cb) {
    ...
    function wait(instId, name, time, startTime, next) {
        const cloudapi = cli.tritonapi.cloudapi;
        ...
        cloudapi.waitForMachineAudit({
            ...
            const durStr = common.humanDurationFromMs(duration);
            ...
            vasync.forEachParallel({
                ...
                func: function deleteOne(name, nextName) {
                    cli.tritonapi.deleteInstanceSnapshot({
                        ...
                        const msg = 'Deleting snapshot "%s" of instance "%s"';
                        console.log(msg, name, instId);

                        if (opts.wait) {
                            const timer = Date.parse(res.headers['date']);
                            wait(instId, name, timer, startTime, nextName);
                        } else {
                            nextName();
                        }
                    });
