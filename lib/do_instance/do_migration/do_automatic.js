...
function do_automatic(subcmd, opts, args, cb) {
    ...
    if (opts.mechanism) {
        automaticOpts.mechanism = opts.mechanism;
    }
...
do_automatic.options = [
    ...
    },
    {
        ...
        type: 'string',
        helpArg: 'MECHANISM',
        help: 'Migration mechanism. "cold" (default) stops the instance ' +
            'during the cutover; "live" hands off between the source and ' +
            'destination hypervisors with a short paused window. `live` ' +
            'requires brand=bhyve.'
    },
    {
        names: ['wait', 'w'],
        type: 'bool',
