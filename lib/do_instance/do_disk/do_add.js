...
function do_add(subcmd, opts, args, cb) {
	...
	// We have to assign block size.
	block_size = jsprim.parseInteger(args[2]);
	if (typeof (block_size) !== 'number') {
            cb(new errors.UsageError('BLOCK_SIZE must be a number if present'));
            return;
	}
    }
...
do_add.synopses = ['{{name}} {{cmd}} [OPTIONS] INST SIZE [BLOCK_SIZE]'];
...
do_add.help = [
    ...
    '    BLOCK_SIZE  (Optional) Size of a block/record',
