...
var OPTIONS = [
     ...
     *
     * See TritonDataCenter/node-triton#28 for some details.
     */
    {
        names: ['account', 'a'],
        type: 'string',
...
CLI.prototype.init = function (opts, args, callback) {
    ...
    /*
     * This option is hidden, and I don't think anyone at Joyent uses it anymore
     * But let it stick around for a while. Removing it should require a
     * major version bump.
     */
    if (opts.url && opts.J) {
    ...
    this.log.debug('Using configDir ' + this.configDir);
            ...
            try {
                    ...
                    !opts.profile && !self.config.profile)
                {
                    ...
                    pErr.message += '\n'
                        ...
                        + '    See https://github.com/TritonDataCenter/node-triton#setup for more help.';
                    /* END JSSTYLED */
                }
                throw pErr;
            }
...
CLI.prototype.do_changefeed = require('./do_changefeed');
...
// Access keys
CLI.prototype.do_accesskey = require('./do_accesskey');
CLI.prototype.do_accesskeys = require('./do_accesskeys');
